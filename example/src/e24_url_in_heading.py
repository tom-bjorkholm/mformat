...
def example_url_in_heading(format_name: str, file_name: str) -> None:
    ...
    with create_mf(format_name=format_name, file_name=file_name) as mf:
        ...
        mf.start_heading(level=1, text='URL in heading example')
        # Start a heading with a URL.
        mf.start_heading(text='A heading with a URL:', level=1)
        # Add a URL to the heading.
        mf.add_url(text='This URL link to the examples.',
                   url=EXAMPLES_URL)
        # Then another heading with a bold URL
        mf.start_heading(text='A heading with a bold URL:', level=2)
        ...
        mf.start_heading(text='A heading with an italic URL:', level=2)
        ...
        mf.start_heading(text='And with an italic and bold URL:', level=2)
        ...
        mf.start_paragraph(text='The add_url function can be used')
