...
def example_nest_numbers_bullets(format_name: str, file_name: str) -> None:
    ...
    with create_mf(format_name=format_name, file_name=file_name) as mf:
        ...
        mf.new_heading(level=1, text='Nesting points example')
        ...
        mf.new_bullet_item('Second bullet is italic', italic=True)
        mf.add_text('with some bold', bold=True)
        mf.add_text('and some non-bold and non-italic text')
        mf.new_numbered_point_item('First item in third level', level=3)
        mf.new_numbered_point_item('Second item in third level')
        # By specifying a lower level we end some nested lists.
        mf.new_numbered_point_item('Third item on first level.', level=1)
