...
def bullets_example(format_name: str,
    ...
    with create_mf(format_name=format_name, file_name=file_name) as mf:
        ...
        mf.start_bullet_item('This is the second bullet point item.', level=2)
        mf.add_text('This time at level 2.')
        mf.start_bullet_item('Another point item without specifying level.')
        mf.add_text('This means that it is at the same level as the previous'
                    ' item.')
        mf.start_bullet_item('This is a bullet point item at level 3.',
                             level=3)
        mf.start_bullet_item('This is final bullet point item.',
                             level=1)
        mf.add_text('This time at level 1')


if __name__ == "__main__":
    example_main(example_text='Headings', function=bullets_example)
